...
object GlobalUtil {
    ...
    val appVersionName: String
        get() = MyApplication.context.packageManager.getPackageInfo(appPackage, 0).versionName
    ...
    fun getDeviceSerial(): String {
        if (deviceSerial == null) {
            ...
            if ("google" != appChannel || "samsung" != appChannel) {
                try {
                    deviceId = Settings.Secure.getString(MyApplication.context.contentResolver, Settings.Secure.ANDROID_ID)
                } catch (e: Exception) {
            ...
            var uuid = MMKV.defaultMMKV()!!.decodeString("uuid", "")
    ...
    fun getResourceId(name: String, type: String): Int {
        return MyApplication.context.resources.getIdentifier(name, type, appPackage)
    }
    ...
    fun getApplicationMetaData(key: String): String? {
        ...
        try {
            applicationInfo = MyApplication.context.packageManager.getApplicationInfo(appPackage, PackageManager.GET_META_DATA)
